...
<!--   Syllabus for PSYC 245: -->
<!-- subtitle: | -->
<!--  Introduction to Psychological Science Methods (Fall 2021) -->
<!-- output:  -->
...
LeaderInfo %>% 
  ...
  pack_rows("Undergraduate Teaching Scholar", 2, 2, background = "#D6D3C4") %>%
  row_spec(0:0, bold = T, color = "white", background = "#500000") %>%
  kable_styling(bootstrap_options = "responsive", font_size=14, full_width = F, position = "left")
...
$\dagger$ Exceptions are during the week of 6-10 September, when Dr. Bolger will hold office hours on Monday, 6 September, from 10am-12pm, and during the week of 4-8 October, when he will also hold office hours on Monday, 4 October, from 10am-12pm.

$\dagger\dagger$ See the end of this syllabus for more information on online student hours
...
Tuesdays and Thursdays from 11:10 am to 12:25 pm in CHEM 100. 

There is no credit for attendance in this class at any point, and the class lectures will be made available synchronously and asynchronously for students who may have to quarantine or who are otherwise indisposed.
...
And due to the need for contact-tracing at the University, we have been encouraged to have assigned seating. This will be figured out on the first day of class.
...
These exams will all be online through the [Respondus Lockdown Browser](https://web.respondus.com/he/lockdownbrowser/). 
...
The first three exams will be due within 1 class meeting of the end of the respective unit. The final exam has a specific date.  

The first three exams will have a 1-hour limit, once started in the *Lockdown Browser*. The final exam will have a 2.5-hour limit. 
...
The last `r G`, however, are conceptually harder and are worth `r E` points. 
...
The last `r K` (Chapters 21-23),$^{\dagger\dagger\dagger}$ however, are conceptually harder and are worth `r J`. 
All together, these sum to `r (H*I)+(J*K)` points, or `r paste0(round((sum((H*I),(J*K))/N)*100,0),"%")` of your total grade). 
...
The post-class quizzes are scored differently than the *Learning Curve* quizzes. Specifically, they are not "all or nothing." Rather, they are penalized. So if you answer the question wrong, you can re-take the question, but you lose 20% of the value of the question. After 5 mistakes, it's worth zero points (penalized 100%). There are multiple questions. On most of the post-class quizzes, there are around 10-15 questions.

There is no time limit to completing these other than the deadline. You can submit the quizzes late, even, but they lose 25% of their total value for each day late.
...
|-------------:|:--------------------:|:--------------------:|:------------:|:----:|
| Exam 1 (Part 1) | `r A1` | `r B1` | `r A1*B1` | `r paste0(round(((A1*B1)/N)*100,0),"%")` |
| Exam 2 (Part 2) | `r A2` | `r B2` | `r A2*B2` | `r paste0(round(((A2*B2)/N)*100,0),"%")` |
| Exam 3 (Part 3) | `r A3` | `r B3` | `r A3*B3` | `r paste0(round(((A3*B3)/N)*100,0),"%")` |
| Final Exam (Part 4) | `r A4` | `r B4` | `r A4*B4` | `r paste0(round(((A4*B4)/N)*100,0),"%")` |
...
pandoc.table(Schedule, style = "multiline", split.table = Inf, split.cells = c(4,7,10,20,15,13), missing = "")
...
pack_rows("Part 1: Producing Data", 3, 8, background = "#D6D3C4") %>%
pack_rows("Part 2: Organizing Data", 9, 15, background = "#D6D3C4") %>%
pack_rows("Part 3: Chance", 15, 22, background = "#D6D3C4") %>%
pack_rows("Part 4: Inference & Ethics", 23, 29, background = "#D6D3C4") %>%
row_spec(0:0, bold = T, color = "white", background = "#500000") %>%
row_spec(30:30, color = "black", background = "grey") %>%
...
$\dagger$ Starting at 12:25 pm, after class has ended  
$\ddagger$ Due at 11:10 am, before class starts  
$\S$ The final exam window begins on Friday, 10 December 2021 at 8:00 am, and ends on Wednesday, 15 December 2021 at 5:00 pm  
...
**Note**  
- You won't be able to access tests with a standard web browser  
...
Due to the need for contact-tracing in the case of students or faculty contracting Covid-19, the university has encouraged professors to implement assigned seating, which will make the process much easier.

We will figure this out on the first day of class.
...
# DETAILS ON STUDENT HOURS

Times: Every week on Wednesday, until Dec 8, 2021, from 8:45-10:45 am$^{\dagger\dagger}$ 

[Join Student Hours on Zoom](https://tamu.zoom.us/j/94664384178?pwd=YVhpK3VxaEVkNnFEak41SllHVStDUT09
)  

$\dagger\dagger$ Exceptions are as follows:

- week of 6-10 September  
  - *Student Hours* on Monday, 6 September, from 10am-12pm  
- week of 4-8 October  
  - *Student Hours* on Monday, 4 October, from 10am-12pm  

Meeting ID: 979 4862 8196  
Passcode: 830339 One tap mobile  
+13462487799,,97948628196# US (Houston)  
+12532158782,,97948628196# US (Tacoma)  
...
Meeting ID: 979 4862 8196  
[Find your local number](https://tamu.zoom.us/u/abcK2FjmXR)  

Join by SIP  
97948628196@zoomcrc.com  
...
Meeting ID: 979 4862 8196  
Passcode: 830339  
[Join by Skype for Business](https://tamu.zoom.us/skype/97948628196)  
